        ...
        Config{
            raw: {
                format!(
                    ...
                    fs::read_to_string(&theme.conf).expect("Unable to read from theme config file").as_str()
                )
            },
            hyprpaper_raw: {
                if theme.hyprpaper.exists(){
                    format!(
                        "#====== generated by hyprtheme ======#\n$THEME_DIR={}\n\n{}",
                        theme.conf.parent().unwrap().to_str().unwrap(),
                        fs::read_to_string(&theme.hyprpaper).expect("Unable to read from theme config file").as_str()
                    )
                } else {
                    String::new()
                }
